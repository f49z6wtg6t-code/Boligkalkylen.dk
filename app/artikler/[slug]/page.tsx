...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  // Title: keep article title + brand suffix within 60 chars
  const suffix = " — BoligKalkylen.dk";
  const maxTitleBody = 60 - suffix.length; // 41 chars for the article title
  const titleBody =
    article.title.length > maxTitleBody
      ? article.title.slice(0, maxTitleBody - 1).trimEnd() + "…"
      : article.title;

  // Description: max 155 chars
  const description =
    article.excerpt.length > 155
      ? article.excerpt.slice(0, 154).trimEnd() + "…"
      : article.excerpt;

  return {
    title: `${titleBody}${suffix}`,
    description,
    alternates: { canonical: `https://boligkalkylen.dk/artikler/${slug}` },
  };
}
...
export default async function ArticlePage({ params }: Props) {
  ...
  const ctaText =
    article.relatedCalculator === "/solceller"
      ? "Prøv vores gratis solcelleberegner og se din besparelse"
      : "Beregn prisen på dit badeværelse her — gratis og uforpligtende";
  ...
  const articleJsonLd = {
    ...
    publisher: {
      ...
      logo: {
        "@type": "ImageObject",
        url: "https://boligkalkylen.dk/logo-header.png",
      },
    },
    mainEntityOfPage: {
      "@type": "WebPage",
      "@id": `https://boligkalkylen.dk/artikler/${slug}`,
    },
  };
            ...
            {article.relatedCalculator && (
              ...
              >
                ...
                <Link
                  ...
                  className="inline-flex items-center gap-2 mt-3 px-5 py-2.5 rounded-xl text-sm font-semibold transition-colors duration-150"
